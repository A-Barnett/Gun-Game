...
public class SniperController : MonoBehaviour
{
    ...
    [SerializeField] private float adsTimeReturn;
    [SerializeField] private float aimYPos;
    [SerializeField] private float aimYRotate;
    [SerializeField] private float fireRate;
    [SerializeField] private float shotForce;
    [SerializeField] private float bulletSpread;
    [SerializeField] private float aimSpreadReduction;
    ...
    [SerializeField]private Animator sniperAnimation;
    ...
    [SerializeField]private float fovDivide;
    ...
    private  Quaternion targetRotation = Quaternion.Euler(new Vector3(70f, 180, 0));
    private int maxAmmo;
    ...
    void FixedUpdate()
    {
        ...
        if (scrollAmount > 0)
        {
            float fov = scopeCam.fieldOfView - zoomSensitivity;
            scopeCam.fieldOfView = Mathf.Clamp(fov, minFov, maxFov);
        }
        ...
        if (inGameMode)
        {
             if (Input.GetAxis("Reload") > 0|| (Input.GetMouseButton(0)&& ammoCount==0))
             {
                 ...
                 {
                     isReloading = true;
                     sniperAnimation.ResetTrigger("Fire");
             ...
             {
                 sniperAnimation.ResetTrigger("Reload");
             }
        }
    ...
    private void HandleAiming()
    {
        if (Input.GetMouseButton(1))
        {
            ...
            transform.localPosition = Vector3.Lerp(transform.localPosition, origWeaponPos + new Vector3(-0.1520973f, aimYRotate, -0.25f), Time.deltaTime * adsTime);
    ...
    private void Fire()
    {
        sniperAnimation.SetTrigger("Fire");
        ...
        if (PlayerController.isAiming)
        {
            ...
            crossHairAnimation.Play("Fire", 0, 0f);
        }
        else
        {
    ...
    private void HandleLooking()
    {
        ...
        if (PlayerController.isAiming)
        {
            float fovMulti = Mathf.Clamp01(scopeCam.fieldOfView / fovDivide);
            PlayerController.gameObject.transform.Rotate(Vector3.up, mouseX * sensitivity * adsSensitivityMulti*fovMulti);
            clampedY = cam.transform.localEulerAngles.x - mouseY * sensitivity * adsSensitivityMulti*fovMulti;
        }
        else
        {
    ...
    private void Reload()
    {
        sniperAnimation.SetTrigger("Reload");
        StartCoroutine(ReloadCock());
        ...
        Vector3 temp = newMag.transform.localScale;
        temp.x *= -1f;
        newMag.transform.localScale = temp;
    ...
    private IEnumerator ReloadCock()
    {
